...
The `@As` annotation can be used to override the shown sentence. To reference arguments use `$`, enumerate them `$1, $2, ...`, or use `$[argumentname]` to have a direct reference.

[source,java]
----
@As ( "the $2 cooked rice bowls and the $eggs fresh eggs" )
