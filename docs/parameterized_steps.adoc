...
This custom annotation will be itself annotated with the `@NamedFormatSet` which will wraps as much as `@NamedFormat` as there are fields needing a specific formatting. +
...
[source,java]
----
public SELF the_coffee_price_$_is_registered(CoffeeWithPrice price ) {
  ...
}
----
