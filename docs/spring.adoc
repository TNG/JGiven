...
:springjunit5javadocurl: http://static.javadoc.io/com.tngtech.jgiven/jgiven-spring-junit5/{version}/com/tngtech/jgiven/integration/spring/junit5
...
If you cannot or don't want to inherit from these classes, you can annotate your test base class with the link:{springjunit5javadocurl}/JGivenSpringExtension.html[`JGivenSpringExtension`]
in addition to Spring's `SpringExtension`.
This extension combines JGiven's regular JUnit 5 lifecycle with installing JGiven's `SpringStageCreator` for the test instance, so you no longer have to copy the content of the base classes:

[source,java]
----
@ExtendWith({SpringExtension.class, JGivenSpringExtension.class})
class MySpringTestBase {
   //...
}
----

The extension is also registered automatically by the base classes above, which means JGiven's stage injection keeps working even if a test declares
`@TestExecutionListeners` with the default `REPLACE_DEFAULTS` merge mode.
