...
Both classes contain all the annotations and methods necessary to run JGiven with Spring. The `SimpleSpringScenarioTest` allows to have all stages in a single stage class, while the `SpringScenario-Test` allows to have a class for each stage.
