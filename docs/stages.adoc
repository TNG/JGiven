...
----
In additon, the `@As` annotation allows declaring an `AsProvider` that handles the step name generation. A default provider can be set via `@JGivenConfiguration` annotation in the class referenced there.
