...
to generated data tables. In general, the preferred way to change the step name should be the `@As` annotation.
...
Please note that a lifecyle model that reuses the same instance to execute multiple test methods is only partially compatible with JGiven. In particular, in such a model stages for different scenarios will be injected into the same field. This can lead to errors when attempting to run tests in parallel.
