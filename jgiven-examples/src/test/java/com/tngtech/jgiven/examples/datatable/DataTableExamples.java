...
public class DataTableExamples extends SimpleScenarioTest<DataTableExamples.DataTableStage> {
    ...
    @NamedFormatSet( { @NamedFormat( name = "name", format = @Format( value = ToUpperCaseFormatter.class ) ),
        @NamedFormat( name = "email", customFormatAnnotation = QuotedCustomFormatAnnotationChain.class ) } )
    @Retention( RetentionPolicy.RUNTIME )
    ...
    static class DataTableStage extends Stage<DataTableStage> {
        ...
        public DataTableStage a_list_of_POJOs_is_used_as_parameters_and_some_fields_are_formatted_using_inline_specified_named_formats(
                @Table( fieldsFormat = {
                    @NamedFormat( name = "name", customFormatAnnotation = UpperCasedCustomFormatAnnotationChain.class ),
